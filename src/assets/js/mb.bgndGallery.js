...
( function( jQuery ) {
	...
	jQuery.mbBgndGallery = {
		...
		buildGallery: function( options ) {
			...
			var pos = el.opt.containment == "main" ? "fixed" : "absolute";
			var css = {
				position: pos,
				top: 0,
				let: 0,
			...
			if( containment != "main" && jQuery( containment ).text().trim() != "" ) {
		...
		checkSize: function( image, el ) {
			...
			return image.each( function() {
				...
				var containment = el.opt.containment == "main" ? window : el.opt.containment;
				var aspectRatio = w / h;
		...
		runFullscreen: function( el ) {
			...
			jQuery( document ).on( fullscreenchange, function() {
				...
				if( !isFullScreen ) {
					...
					if( !jQuery( el.opt.containment ).is( "main" ) )
